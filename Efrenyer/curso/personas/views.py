...
def personas_borrar01(request):
   ...
   if request.method == 'GET':
        try:
           ...
           mensaje = "El contacto fue eliminado con exito." 
           tipo=1
...
def personas_actualizar01(request):
  ...
  try:
    ...
    correo = contacto.correo
    telefono = contacto.telefono
    mensaje = 'Consulta procesada con éxito, proceda con la modificacion del registro'
    tipo = 1
    ...
    mensaje = 'No se tienen datos almacenados para actualizar ' 
    tipo = 3 
...
def personas_actualizar02(request):
  ...
  if request.method == 'POST':
    try:
      ...
      mensaje = 'Contacto fue actualizado con exito.'
      tipo = 1
    except Exception as e:
      mensaje = 'Ocurrió un error al procesar la consulta: ' + str(e)
      tipo = 2
